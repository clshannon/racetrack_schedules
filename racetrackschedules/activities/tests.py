...
class ActivityTest(WebTest):
    ...
    def test_list_view(self):
        ...
        """
        instance = mommy.make(Activity)
        response = self.app.get(reverse('activities:list'))
    ...
    def test_create_view(self):
        ...
        """
        response = self.app.get(reverse('activities:create'))
        new_name = 'A freshly created thing'

        # check that we don't already have a model with this name
        self.assertFalse(Activity.objects.filter(name=new_name).exists())

        form = response.forms['activity_form']
        form['name'] = new_name
        form.submit().follow()

        instance = Activity.objects.get(name=new_name)
    ...
    def test_update_view(self):
        ...
        """
        instance = mommy.make(Activity)
        response = self.app.get(reverse('activities:update', kwargs={'pk': instance.pk, }))
    ...
    def test_delete_view(self):
        ...
        instance = mommy.make(Activity)
        pk = instance.pk
        response = self.app.get(reverse('activities:delete', kwargs={'pk': pk, }))
